...
packet_resource_pack_client_response:
   !id: 0x08
   !bound: server
   # As of 1.26.40 the status is an unsigned varint and no longer includes the 'none' entry. It is
   # followed by the same status as a string, and the pack IDs are only written for 'send_packs'.
   response_status: varint =>
      0: refused
      1: send_packs
      2: have_all_packs
      3: completed
   ...
   # All of the pack IDs.
   resourcepackids: response_status ?
      if send_packs: ResourcePackIds
      default: void
...
packet_start_game:
   ...
   # As of 1.26.40 this is an unsigned varint.
   edu_offer: varint
   ...
   # As of 1.26.40 StartGame uses the same game rule encoding as the rest of the protocol (int values
   # are little endian ints instead of varints).
   gamerules: GameRuleI32[]varint
   ...
   # IsLoggingChat, which indicated that the server owner was logging chat messages on the server machine
   # (Education Edition only), was removed in 1.26.40.
   # New fields 1.26.0
...
packet_move_player:
   ...
   # As of 1.26.40 the teleport data is prefixed with a boolean instead of being inferred from the mode.
   teleport?:
...
packet_crafting_data:
   !id: 0x34
   !bound: client
   # As of 1.26.40 recipes are no longer sent as one tagged array; each recipe kind has its own array and
   # the recipe type is implied by which array the recipe appears in.
   shaped_recipes: ShapedRecipe[]varint
   shapeless_recipes: ShapelessRecipe[]varint
   multi_recipes: MultiRecipe[]varint
   # Shulker box recipes, which are shapeless.
   shulker_box_recipes: ShapelessRecipe[]varint
   shapeless_chemistry_recipes: ShapelessRecipe[]varint
   shaped_chemistry_recipes: ShapedRecipe[]varint
   smithing_transform_recipes: SmithingTransformRecipe[]varint
   smithing_trim_recipes: SmithingTrimRecipe[]varint
...
packet_level_chunk:
   ...
   # As of 1.26.40 the sub-chunk request mode is no longer encoded in SubChunkCount; instead the highest
   # sub-chunk is sent as an optional. It is only present if the sub-chunk request mode is limited.
   highest_subchunk_count?: zigzag32
   ...
   # of the chunk.
   # If CacheEnabled is set to false, BlobHashes is left empty. As of 1.26.40 the array is always written.
   blobs: lu64[]varint
...
packet_clientbound_map_item_data:
   ...
   origin: vec3i
   # As of 1.26.40 the UpdateFlags bitfield is gone: every section below is written as its own optional.
   ...
   tracked_objects?: TrackedObject[]varint
...
packet_transfer:
   ...
   port: lu16
   reload_world: bool
   gatherings_configuration?: GatheringJoinInfo
...
packet_play_sound:
   ...
   handle?: lu64
   playback_position_seconds?: lf32
...
packet_structure_block_update:
   ...
   # As of 1.26.40 this is a single byte instead of a zigzag varint.
   redstone_save_mode: u8
...
packet_set_score:
   !id: 0x6c
   !bound: client
   # As of 1.26.40 the packet-wide action byte is gone. Every entry carries its own type, written both as
   # an unsigned varint and as a string, and the payload that follows depends on that type.
   entries: []varint
      entry_type: varint =>
         0: remove
         1: player
...
packet_move_entity_delta:
   ...
   runtime_entity_id: varint64
   # As of 1.26.40 the DeltaMoveFlags bitfield is gone; every component is a separate optional and the
   # remaining flags are written as plain booleans at the end of the packet.
   x?: lf32
   y?: lf32
   z?: lf32
   rot_x?: u8 # TODO: * implement ByteFloat
   rot_y?: u8
   rot_z?: u8
   on_ground: bool
   force_move: bool
...
packet_player_auth_input:
   ...
   head_yaw: lf32
   # InputData specifies the way the player moved last tick. As of 1.26.40 it is an optional list of
   # InputData values instead of a bitfield.
   input_data: InputData[]varint
   ...
   delta: vec3f
   # As of 1.26.40 none of the fields below are gated on the input flags any more. Each is preceded by a
   # presence byte (always written as true) and is then written as its own optional.
   transaction?:
      legacy: TransactionLegacy
      # The actions carry their own presence byte and optional, unlike the plain
      # optional the InventoryTransaction packet wraps them in.
      actions: TransactionActions
      data: TransactionUseItem
   item_stack_request?: ItemStackRequest
   block_action?: []varint
      action: Action
      # As of 1.26.40 the position and face are always written, whatever the action is.
      # BlockPosition is the position of the target block, if the action with the ActionType set concerned a
      # block. If that is not the case, the block position will be zero.
      position: vec3i
      # BlockFace is the face of the target block that was touched. If the action with the ActionType set
...
# As of 1.26.40 the input flags are no longer a bitfield: they are sent as an optional list of the
# ordinals below. The values are unchanged from the old bit positions.
InputData: zigzag32 =>
   0: ascend
   1: descend
   2: north_jump
...
packet_creative_content:
   ...
   groups: []varint
      ...
      # As of 1.26.40 this is a single byte instead of a little endian int.
      category: u8 =>
         - all
         - construction
         - nature
...
# As of 1.26.40 there is a single entry layout: the payload, both heightmaps and the blob ID are all
# optionals, so they no longer depend on the result or on whether the blob cache is enabled.
SubChunkEntry: []varint
   dx: i8
   dy: i8
   dz: i8
   result: u8 =>
...
packet_serverbound_diagnostics:
   ...
   # Added in 1.26.40.
   system_categories: SystemCategory[]varint
   whisker_scopes: WhiskerScopeDataSummary[]varint
...
packet_player_update_entity_overrides:
   ...
   # As of 1.26.40 it is sent twice: once as an unsigned varint and once as the legacy byte.
   type: varint =>
      - clear_all
      - remove
      - set_int
...
packet_player_location:
   !id: 0x146
   !bound: client
   # As of 1.26.40 the target entity comes first and the type is an unsigned varint.
   entity_unique_id: zigzag64
...
packet_clientbound_update_sound_data:
   ...
   # As of 1.26.40 the string type identifier is replaced by one optional per update kind. Each optional
   # payload starts with an unsigned varint that is currently always zero.
   stop?:
      unused: varint
   volume?:
      unused: varint
...
packet_send_party_destination_cookie:
   !id: 0x15d
   !bound: client
   cookie: string
   intent: string
...
# Sent by both sides to update the tabs, filtering and layout a player selected in a furnace-like container UI.
...
# Sent by the server when a record starts playing at a block, such as a music disc inserted into a jukebox.
# The handle can be used with clientbound_update_sound_data afterwards.
