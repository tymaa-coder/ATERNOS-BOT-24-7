...
# As of 1.26.40 the pack list is prefixed with a varint count instead of a little endian short.
TexturePackInfos: []varint
   uuid: uuid
   version: string
   size: lu64
   content_key: string
...
# As of 1.26.40 an air item (network_id 0) no longer short-circuits: every field is always written, only
# the extra data blob ends up empty.
ItemLegacy:
   network_id: zigzag32
   count: lu16
   metadata: varint
...
# As of 1.26.40 the standalone `Item` encoding is gone: everything that used it now uses `ItemV4`.
# As of 1.26.40 the net ID is a plain zigzag32 again; the oneOf variant tag that preceded it is gone.
ItemV4:
   network_id: li16
   count: lu16
   metadata: varint
...
MetadataDictionary: []varint
   ...
   type: varint =>
      ...
      7: long
      8: vec3f
   # As of 1.26.40 the value type is sent twice: once as an unsigned varint and once as a byte.
   legacy_type: u8
   value: key ?
...
TransactionLegacy:
   ...
   legacy_transactions?: []varint
      container_id: u8
      changed_slots: []varint
         slot_id: u8

# As of 1.26.40 the inventory transaction embedded in PlayerAuthInput was brought back in line with the
# one in InventoryTransaction, so it reuses `TransactionLegacy`/`TransactionActions` above again.
...
ItemStacks: ItemV4[]varint

# As of 1.26.40 the descriptor is identified by a string rather than by the numeric tag. The leading
# varint only distinguishes 'no descriptor' (0) from 'a descriptor follows' (1); when it is 1 the actual
# kind is read from the descriptor_type string. The deferred and complex alias descriptors are gone.
RecipeIngredient:
   type: varint =>
      0: invalid
      1: valid
      ...
      if valid:
         ...
         _: descriptor_type ?
            ...
            if name:
               ...
               name: string
               # MetadataValue is the metadata value of the item. For some items, this is the damage value,
               # whereas for other items it is simply an identifier of a variant of the item.
               metadata: zigzag32
            ...
            if molang:
               ...
               version: li16
            # ItemTagItemDescriptor represents an item descriptor that uses item tagging. This should be used to
            # reduce duplicative entries for items that can be grouped under a single tag.
            if item_tag:
...
# As of 1.26.40 each recipe kind is sent in its own array in packet_crafting_data, so the recipe type
# tag that used to prefix every entry is gone.
ShapedRecipe:
   recipe_id: LatinString
   width: zigzag32
   height: zigzag32
   # Flat input array, size of width*height
   ...
   # Only written for regular shaped recipes, never for chemistry ones.
...
ShapelessRecipe:
   ...
   block: string
   priority: zigzag32
   # Only written for regular shapeless and shulker box recipes, never for chemistry ones.
   unlocking_requirement?: RecipeUnlockingRequirement
   network_id: varint
...
# SmithingTransformRecipe is a recipe specifically used for smithing tables. It has two input items and adds
# them together, resulting in a new item.
...
# As of 1.26.40 the context is a zigzag32 and the ingredient list is written as an optional (present only
# when the context is 'none') rather than being inferred from the context alone.
...
# 1.26.40 reworked most of this type: the three arrays are now varint-counted, the arm size and the piece
# types are numeric, the colours are little endian ints instead of strings, and the trusted flag and the
# profile hash were appended at the end.
PersonaPieceType: li32 =>
   - unknown
   - skeleton
   - body
   - skin
...
Skin:
   ...
   animations: []varint
      ...
      animation_type: varint
      animation_frames: lf32
      expression_type: varint
   cape_data: SkinImage
   ...
   # Added in 1.26.40. Sent as the string "true" or "false".
   trusted: string
   # Added in 1.26.40.
   profile_hash: string
...
StackRequestSlotInfo:
   ...
   # As of 1.26.40 this is a little endian int instead of a zigzag varint.
   stack_id: li32

# The ingredient encoding used inside auto-craft item stack request actions. It differs from
# `RecipeIngredient` in that the descriptor kind is numeric (and repeated as a byte) and the count is a
# little endian short.
RecipeIngredient2:
   type: varint =>
      0: invalid
      ...
      2: molang
      3: item_tag
   # The descriptor kind again, as a byte. The reader ignores it in favour of the varint above.
   legacy_type: u8
   _: type ?
      if invalid: void
      if name:
...
# The item encoding used by the deprecated craft-results item stack request action. Unlike `ItemLegacy` it
# identifies the item by name through a descriptor rather than by network ID.
ItemStackRequestInstanceDescriptor:
   type: varint =>
      0: invalid
      1: name
   # The descriptor kind again, as a byte.
   legacy_type: u8
   _: type ?
      if invalid: void
      default:
   ...
   count: li16
   block_runtime_id: varint
   extra: '["encapsulated", { "lengthType": "varint", "type": "ItemExtraDataWithoutBlockingTick" }]'
...
ItemStackRequest:
   ...
   request_id: zigzag32
   actions: []varint
      # As of 1.26.40 the action type is an unsigned varint, the 'place in container' and 'take out
      # container' entries are gone, and the type is repeated as a byte right after it.
      type_id: varint =>
         ...
         # crafting, where the old item is consumed.
         - results_deprecated
      # The action type again, as a byte.
      legacy_type_id: u8
      _: type_id ?
         if take or place:
            count: u8
         ...
         if mine_block:
            ...
            # must check if these IDs match. If they do not match, servers should reject the stack request that the
            # action holding this info was in.
            # As of 1.26.40 this is a little endian int.
            network_id: li32
         if craft_recipe:
         ...
         if craft_recipe_auto:
            ...
            # one of the recipes sent in the CraftingData packet, where each of the recipes have a RecipeNetworkID as
            recipe_network_id: varint
            # As of 1.26.40 the duplicated craft count is gone and the ingredients use their own encoding.
            times_crafted: u8
            ingredients: RecipeIngredient2[]varint
         if craft_creative:
         ...
         if craft_grindstone_request:
            # RecipeNetworkID is the network ID of the recipe that is about to be crafted. This network ID matches
            # one of the recipes sent in the CraftingData packet, where each of the recipes have a RecipeNetworkID as
            # As of 1.26.40 it is a little endian int here.
            recipe_network_id: li32
         ...
         if results_deprecated:
            result_items: ItemStackRequestInstanceDescriptor[]varint
            times_crafted: u8
...
ItemStackResponses: []varint
   ...
   request_id: zigzag32
   # As of 1.26.40 the container list no longer depends on the status: it is preceded by a presence byte
   # (always written as true) and is then written as an optional.
   # ContainerInfo holds information on the containers that had their contents changed as a result of the
   # request. Absent when there are no containers to report.
   containers?: []varint
      ...
      slots: []varint
         ...
         # sent to the client.
         count: u8
         # StackNetworkID is the network ID of the new stack at a specific slot. As of 1.26.40 it is preceded
         # by a presence byte (always written as true) and is then written as an optional.
...
TrackedObject:
   ...
   type: li32 =>
      0: entity
      1: block
   # As of 1.26.40 both members are written as optionals rather than being selected by Type.
...
MapDecoration:
   ...
   # automatically, whereas others may be changed. As of 1.26.40 this is a little endian int.
   color_abgr: li32
...
BiomeChunkGeneration:
   ...
   village_type: has_village_type ?
      if true: u8
   # As of 1.26.20 the surface descriptions moved out of this struct into a pair of surface builders at the
   # end of it. SurfaceBuilder describes the top layer, SubSurfaceBuilder the layer underneath it.
   surface_builder_data?: BiomeSurfaceBuilder
   sub_surface_builder_data?: BiomeSurfaceBuilder

# BiomeSurfaceBuilder describes how one layer of a biome's surface is generated.
BiomeSurfaceBuilder:
   ...
   # NoiseGradientSurface drives the surface from a named noise function. Added in 1.26.20.
   noise_gradient_surface?: BiomeNoiseGradientSurface

# BiomeNoiseGradientSurface generates a surface by sampling a named noise function.
BiomeNoiseGradientSurface:
   non_replaceable_blocks: lu32[]varint
   gradient_blocks: BiomeNoiseBlockSpecifier[]varint
   noise_seed: string
   first_octave: lu32
   amplitudes: lf32[]varint

# BiomeNoiseBlockSpecifier picks a block based on where a noise sample falls in a range. As of 1.26.30 this
# replaced the plain block runtime id previously used for `gradient_blocks`.
...
WhiskerScopeDataSummary:
   label: string
   indentation: string
   total_high_cost_ns: lu64
   total_mid_cost_ns: lu64
   total_low_cost_ns: lu64
...
# Added in 1.26.50: sub chunk height maps are sent as 16 rows of 16 heights, each row length-prefixed.
HeightMap: '["array", { "count": 16, "type": ["array", { "countType": "varint", "type": "i8" }] }]'

# Added in 1.26.50: the options a player selected in a furnace-like container UI.
FurnaceOptions:
   left_furnace_tab: zigzag32
   filtering: bool
   layout: zigzag32
